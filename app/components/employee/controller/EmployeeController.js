...
class EmployeeController {
    ...
    async createEmployee(settingsConfig, req, res, next) {
        try {
            ...
            const { employeeName, username, password, email } = req.body
    ...
    async getAllEmployeeEmail(settingsConfig, req, res, next) {
        try {
            const logger = settingsConfig.logger;
            const queryParams = req.query
            logger.info(`[EMPLOYEE_CONTROLLER] : Inside getAllEmployee`);
            const data = await this.newEmployeeService.getAllEmpoyeeEmail(settingsConfig, queryParams)
            // res.set('X-Total-Count', count)
            res.status(HttpStatusCode.Ok).json(await (data.rows.map(employee => employee.email)))

            const otherServiceEndpoint = 'http://localhost:3001/api/receiveData';

            await axios.post(otherServiceEndpoint, { dataArray: data.rows.map(employee => employee.email) });

            return data.rows.map(employee => employee.email);
        } catch (error) {
            next(error)
        }
    }
